...
import { formatNodeValue, spacedTitle, toCapitalize } from '../../util/styled';
    ...
    const recursivePush = (model: Models, title: string) => {
		...
		Object.keys(model).forEach((key) => {
			if (typeof model[key as keyof Models] !== 'object') {
				gridNodes.push( { label: key, value: formatNodeValue(key, model[key as keyof Models]) } );
			} else {
				const subModel = ({...model} as any)[key] as Models;
				recursivePush(subModel, key);
			}
		});
        const formatedTitle = spacedTitle(title);
		gridNodesSections.push({title: formatedTitle, gridNodes});
	}
